...
namespace KawaiiBot
{
    ...
    public class Sample
    {
        ...
        public Sample(double[] inputValues, int classes_count, int sampleClassId = -1)
        {
            //  Клонируем массивчик
            input = (double[])inputValues.Clone();
            output = new double[classes_count];
            if (sampleClassId != -1) output[sampleClassId] = 1;


            recognizedClassId = -1;
            actualClassId = sampleClassId;
        }
        ...
        public void processOutput()
        {
            ...
            {
                error[i] = ((i == actualClassId ? 1 : 0) - output[i]);
                if (output[i] > output[recognizedClassId]) recognizedClassId = i;
            }
        }
        ...
        /// </summary>
        /// <returns></returns>
        public bool Correct() { return actualClassId == recognizedClassId; }
    }
}
